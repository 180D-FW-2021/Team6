#!/usr/bin/python
# based on berryIMU.py in ozzmaker/BerryIMU http://ozzmaker.com/
...
while True:
    ...
    # state machine
    if state == 0 and ACCy > 600:
        state = 1
    elif state == 1 and ACCx > 600:
        state = 2
    elif state == 1 and ACCx < -400:
        state = 3
    ...
    if 0:
        outputString +="GYRx %5.2f GYRy %5.2f GYRz %5.2f " % ( gyroXangle, gyroYangle, gyroZangle)

    if state == 2:
        outputString+= "Page turn left detected"
    elif state == 3:
        outputString+= "Page turn right detected"

    #outputString += str(time.time())
    print(outputString)
    ...
    acc_y.append(ACCy)
    acc_z.append(ACCz)
    gyr_x.append(gyroXangle)
    gyr_y.append(gyroYangle)
    gyr_z.append(gyroZangle)
